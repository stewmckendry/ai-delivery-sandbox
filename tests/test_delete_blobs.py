...
def test_delete_blobs_names(monkeypatch, capsys):
    ...
    deleted = []
    monkeypatch.setattr(module.blob, "delete_blobs", lambda names: deleted.extend(names))
...
def test_delete_blobs_folder(monkeypatch, capsys):
    module = importlib.import_module("scripts.delete_blobs")

    monkeypatch.setattr(module.blob, "list_blobs", lambda prefix: ["a/b", "a/c"])
    deleted = []
    monkeypatch.setattr(module.blob, "delete_blobs", lambda names: deleted.extend(names))

    monkeypatch.setattr(sys, "argv", ["delete_blobs.py", "--folder", "session/1/"])
    module.main()
    out = capsys.readouterr().out
    assert "Deleted 2 blobs" in out
    assert deleted == ["a/b", "a/c"]
