...
def assess_concussion(payload: AssessmentRequest):
    ...
    try:
        ...
        log = db.query(SymptomLog).filter_by(user_id=payload.user_id).order_by(SymptomLog.timestamp.desc()).first()
        if not log:
            raise HTTPException(status_code=400, detail="No symptom log found. Complete triage first.")

        try:
            symptom_scores = json.loads(log.symptoms)
        except Exception:
            raise HTTPException(status_code=500, detail="Failed to parse symptom log.")
